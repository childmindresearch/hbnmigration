...
locals {
  ...
  services = {
    ...
    hbn_sync                   = "${local.service_prefix}hbn-sync"
...
resource "local_file" "hbn_sync_service" {
  ...
  filename   = "${path.module}/.generated/${local.services.hbn_sync}.service"
  depends_on = [null_resource.ensure_user_group]
}
...
resource "null_resource" "enable_services" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      # Enable and start the timer (which will trigger the oneshot services)
      ...
      # Enable and start the always-on websocket service separately
      sudo systemctl enable ${local.services.curious_alerts_websocket}.service
      sudo systemctl restart ${local.services.curious_alerts_websocket}.service 2>/dev/null || sudo systemctl start ${local.services.curious_alerts_websocket}.service

      echo "✓ Services started for workspace: ${terraform.workspace}"
      echo ""
      echo "Timer-triggered services (run every minute):"
      ...
      echo "  - ${local.services.curious_accounts_to_redcap}.service"
      echo "  - ${local.services.curious_data_to_redcap}.service"
      echo ""
